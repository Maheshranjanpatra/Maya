...
(() => {
  ...
  function checkLevelUp() {
    ...
    while (state.xp >= xpForNextLevel()) {
      ...
      if (state.level % 8 === 0) {
        ...
        `);
        speak("Level " + state.level + ". You have earned the right to celebrate. Go out. Have a proper dinner.");
      } else {
        showToast(`Level ${state.level} reached.`);
        speak(`Level ${state.level}. Strength is built in silence. Continue.`);
      }
      triggerConfetti();
    }
  ...
  function speak(text) {
    ...
    try {
      ...
      utter.rate = 0.85;   // slower = more weight
      ...
      let preferred = voices.find(v =>
        /daniel|david|james|mark|george|alex|fred|male|man|baritone/i.test(v.name) &&
        !/female|woman|girl/i.test(v.name)
      );
      if (!preferred) {
        preferred = voices.find(v => v.lang.startsWith('en') && /male|man/i.test(v.name));
      }
      if (!preferred) {
        preferred = voices.find(v => v.lang.startsWith('en-GB') || v.lang.startsWith('en-US'));
      }
  ...
  setInterval(() => {
    ...
    state.todos.forEach(todo => {
      if (!todo.completed && todo.alarm === hhmm && !todo.alarmTriggered) {
        ...
        speak(`It is time. ${todo.title}. Execute.`);
  ...
  function attachEventListeners() {
    ...
    document.getElementById('addHabitBtn')?.addEventListener('click', () => {
      ...
      document.getElementById('saveHabit').onclick = () => {
        ...
        speak('New habit registered. Consistency is the only currency that matters.');
      };
    });
    ...
    document.getElementById('importBtn')?.addEventListener('change', (e) => {
      ...
      reader.onload = (ev) => {
        try {
          ...
          speak('Data restored. Continue from where you left the work.');
        } catch {
  ...
  document.querySelectorAll('.side-link').forEach(link => {
    link.addEventListener('click', () => {
      ...
      if (action === 'profile') {
        ...
        speak(`Profile of ${state.profile.name}. The work continues.`);
      }
      ...
      if (action === 'level') {
        ...
        `);
        document.getElementById('closeModalBtn').onclick = closeModal;
        speak(`Level ${state.level}. Strength compounds. Do not stop.`);
      }
      ...
      if (action === 'sounds') {
        showModal(`
          ...
          <div class="space-y-4">
            ...
            <label class="flex items-center justify-between">
              <span>Deep Voice</span>
              <input type="checkbox" id="sideVoice" ${state.settings.voiceEnabled ? 'checked' : ''} class="w-5 h-5 accent-emerald-500">
            </label>
            <label class="flex items-center justify-between">
              <span>Vibration</span>
          ...
          <button id="testVoiceBtn" class="w-full mt-5 py-3 rounded-xl bg-maya-neon/20 border border-maya-neon/40 text-maya-neon font-semibold">Test Voice</button>
        ...
        document.getElementById('sideVoice').onchange = (e) => { state.settings.voiceEnabled = e.target.checked; saveState(); };
        ...
        document.getElementById('testVoiceBtn').onclick = () => {
          speak("This is the voice. Deep. Clear. Ready when you are.");
          playChime('complete');
        };
        document.getElementById('closeModalBtn').onclick = closeModal;
      }
      ...
      if (action === 'export') {
        ...
        speak('Export or import your data from Settings.');
      }
    });
  });
  ...
  document.getElementById('enableNotifBtn').onclick = async () => {
    ...
    if (ok) speak('You will be reminded when the time comes.');
  };
