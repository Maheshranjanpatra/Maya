/* =====================================================
   ...
   Offline-first, neon dark, girlfriend-style companion
   ===================================================== */
...
(() => {
  ...
  // -------------------- QUOTES (Girlfriend style) --------------------
  const QUOTES = [
    "Hey... you showed up today. That already makes me proud of you.",
    "Even small steps count, okay? I'm right here cheering for you.",
    "You don't have to be perfect. Just keep going. I've got you.",
    "Remember when you thought you couldn't? Look at you now.",
    "Take a deep breath. You're doing better than you think.",
    "I believe in you more than you believe in yourself sometimes.",
    "It's okay to rest. Just don't give up on yourself, alright?",
    "Every habit you complete is a little love letter to your future self.",
    "You're not alone in this. I'm always on your side.",
    "Today is a new chance. Let's make it gentle and good.",
    "You make me want to be better just by watching you try.",
    "Slow progress is still progress. I'm so proud of you.",
    "If it feels heavy today, lean on me a little. I've got strong shoulders.",
    "You're allowed to celebrate the tiny wins. They add up, love.",
    "Keep that fire going. Even a small flame can light the way."
  ];
  ...
  function checkLevelUp() {
    ...
    while (state.xp >= xpForNextLevel()) {
      ...
      if (state.level % 8 === 0) {
        ...
        `);
        speak("Level " + state.level + "! You've earned a nice dinner. I'm so proud of you.");
      } else {
        showToast(`Level ${state.level} reached! You're amazing.`);
        speak(`You reached level ${state.level}. Keep going, love.`);
      }
      triggerConfetti();
    }
  ...
  setInterval(() => {
    ...
    state.todos.forEach(todo => {
      if (!todo.completed && todo.alarm === hhmm && !todo.alarmTriggered) {
        ...
        speak(`Hey love, it's time for ${todo.title}`);
  ...
  function render() {
      ...
      state.habits.forEach(h => {
        if (h.completedToday) {
          ...
          if (state.settings.streakFreezes > 0) {
            ...
            showToast('Streak freeze used. I protected your streak.');
          } else {
            h.streak = 0;
          }
        }
    ...
    document.getElementById('sideAvatar').innerHTML = animeGirlSVG(56);
  ...
  function renderHabits() {
    ...
    }).join('') || `<p class="text-center text-slate-500 py-10">No habits yet. Add your first one, love.</p>`;
      ...
      <div class="mb-6">
        <div class="bg-maya-card border border-maya-border rounded-3xl p-5 flex items-center gap-5 card-glow">
          ...
          <div>
            ...
            <div class="text-xs text-maya-neon mt-1">Keep going, I'm with you</div>
          </div>
        </div>
      </div>
    ...
    return `
      <div class="space-y-4">
        ...
        <div class="bg-maya-card border border-maya-border rounded-2xl p-4 space-y-3">
          ...
          <label class="flex items-center justify-between">
            <span>Voice (Cute Female)</span>
            <input type="checkbox" id="settingVoice" ${state.settings.voiceEnabled ? 'checked' : ''} class="w-5 h-5 accent-maya-neon" />
          </label>
          <label class="flex items-center justify-between">
            <span>Vibration</span>
        ...
        <div class="bg-maya-card border border-maya-border rounded-2xl p-4">
          ...
          <p class="text-xs text-slate-400 leading-relaxed mb-3">
            Notifications work when the app is installed to Home Screen and not force-stopped.<br><br>
            <strong class="text-maya-neon">To keep them reliable:</strong><br>
            • Add Maya to Home Screen<br>
            • Go to Settings → Apps → Maya (or Chrome) → Battery → Unrestricted<br>
            • Avoid force-stopping the app<br>
            • Keep it out of "Sleeping apps"
          </p>
  ...
  function updateNavIndicator(activeItem) {
    ...
    const translateX = centerLeft - 36; // 72px / 2
  ...
  function attachEventListeners() {
    ...
    document.getElementById('addHabitBtn')?.addEventListener('click', () => {
      ...
      document.getElementById('saveHabit').onclick = () => {
        ...
        closeModal();
        render();
        showToast('Habit added. Let’s build this together.');
        speak('New habit added. I’m excited for you.');
      };
    });
      ...
      btn.onclick = (e) => {
        ...
        if (h.completedToday) {
          ...
          if (state.habits.every(x => x.completedToday) && state.habits.length > 0) {
            state.xp += 100;
            triggerConfetti();
            showToast('All habits done today! You absolute star.');
            speak('You finished everything today. I’m so proud of you.');
            checkLevelUp();
          }
        }
        saveState();
    ...
    document.getElementById('settingVoice')?.addEventListener('change', (e) => {
    ...
    document.getElementById('testNotifBtn')?.addEventListener('click', async () => {
      ...
      if (ok) {
        showNotification('Maya', 'Notifications are working, love. I’ll remind you.');
        showToast('Test sent!');
      }
    });
    ...
    document.getElementById('importBtn')?.addEventListener('change', (e) => {
      ...
      reader.onload = (ev) => {
        try {
          ...
          speak('Welcome back. Everything is just as you left it.');
        } catch {
  ...
  // Close side menu when clicking any side link (for now just close)
  document.querySelectorAll('.side-link').forEach(link => {
    link.addEventListener('click', () => {
      closeSideMenu();
      // Future: open specific panels
    });
  });
  ...
  document.getElementById('enableNotifBtn').onclick = async () => {
    const ok = await requestNotificationPermission();
    showToast(ok ? 'Alerts enabled. I’ll take care of you.' : 'Permission denied');
    if (ok) speak('I’ll remind you when it’s time.');
  };
  ...
  function maybeShowQuote() {
    ...
    if (now - state.settings.lastQuoteTime > 2 * 60 * 60 * 1000) {
      ...
      document.getElementById('quoteAvatar').innerHTML = animeGirlSVG(72);
